...
const DashboardNurseryPage: React.FC<UserEditFormProps> = ({ userData }) => {
  const [selectedUser, setSelectedUser] = useState<CustomOptionType | null>(
    null
  );
  ...
  const userOptions = Object.keys(userData).map((key) => ({
    value: userData[Number(key)].id,
    label: userData[Number(key)].name,
  }));
      ...
      <TabsContent value="nursery" className="flex flex-col gap-2 w-full">
        <div className="input-fields-container">
          ...
          <Select
            ...
            options={userOptions}
            value={selectedUser}
            onChange={(selectedOption: CustomOptionType | null) => {
